...
if (count($parts) === 3 && is_numeric($parts[2])) { $id = (int)$parts[2]; }
$body = file_get_contents('php://input');
$payload = $body ? json_decode($body, true) : [];
try {
    switch ($method) {
        case 'GET':
            if ($id !== null) { $todo = $repo->get($id); $todo ? json_out($todo) : json_out(['error' => 'Not found'], 404); }
            else { json_out($repo->all()); }
            break;
        case 'POST':
            if (!isset($payload['title']) || trim($payload['title']) === '') { json_out(['error' => 'title is required'], 422); }
            $todo = $repo->create($payload['title']); json_out($todo, 201);
            break;
        case 'PUT':
            if ($id === null) json_out(['error' => 'id required'], 400);
            $todo = $repo->update($id, ['title' => $payload['title'] ?? null, 'done' => isset($payload['done']) ? (bool)$payload['done'] : null]);
            $todo ? json_out($todo) : json_out(['error' => 'Not found'], 404);
            break;
        case 'DELETE':
            if ($id === null) json_out(['error' => 'id required'], 400);
            $ok = $repo->delete($id); json_out(['deleted' => $ok]);
            break;
